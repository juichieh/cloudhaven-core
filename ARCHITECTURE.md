...
- 哪些模組目前已啟用
- 哪些模組是預留擴建區
- 如何避免 `skills/`、`frameworks/`、`knowledge/`、`ai-tools/` 互相重複
- 如何避免 `Workflow`、`Action`、`Skill`、`Tool` 名詞打架
...
```text
Workflow = Skill Chain 的描述
Skill    = 真正保存與重用的操作套路
```

如果內容不能被拿來執行，就不要放進 `skills/`。

如果只是 Workflow 名稱，也不要放進 `skills/`。
...
如果它是具體產品、API、MCP、平台，放 `library/ai-tools/`。

如果它是操作這些工具完成任務的流程，放 `skills/`。
...
Room 裡可以描述某個專案如何串接多個 Skill。

但 Room 不應該把全域知識、全域 Skill 或 Library 卡片重複複製進去。

如果需要描述 Workflow，優先在 Room 裡用文字描述 Skill Chain，不要另外建立 `workflow/`。
